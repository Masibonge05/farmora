...
const topNavItems = [
  ...
  { id: 'farmora-ai', label: 'AI Assistant', icon: Bot },
];
...
const mobileNavItems = [
  ...
  { id: 'weather', label: 'Weather', icon: CloudRain },
  { id: 'market', label: 'Market', icon: ShoppingCart },
  { id: 'farmora-ai', label: 'AI', icon: Bot },
];
...
export default function App() {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--bg-1)' }}>
      ...
      <main style={{ flex: 1, overflowY: 'auto', position: 'relative', zIndex: 1, paddingBottom: isMobile ? 78 : 0 }}>
        <div
        ...
        <nav className="mobile-bottom-nav" aria-label="Mobile primary navigation">
          {mobileNavItems.map(({ id, label, icon: Icon }) => (
            <button
              key={id}
              type="button"
              className={`mobile-bottom-item ${activePage === id ? 'active' : ''}`}
              onClick={() => handleNav(id)}
              aria-label={label}
            >
              <Icon size={16} />
