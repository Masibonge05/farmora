...
const BatteryIcon = ({ level }) => {
  const color = level < 20 ? '#c85820' : level < 50 ? '#e8a020' : '#b8924a';
  return (
...
const signalColor = { Strong: '#b8924a', Good: '#9d7a3d', Weak: '#e8a020', Poor: '#c85820', None: '#666' };
  ...
  return (
    <div style={{ padding: isMobile ? '18px 14px' : '28px 32px', maxWidth: 1100 }}>
      <div style={{ marginBottom: 28 }}>
        ...
        <div style={{ fontSize: 14, color: '#6b7280' }}>ESP32-based sensor nodes - Real-time environmental monitoring</div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : isTablet ? 'repeat(2, 1fr)' : 'repeat(4, 1fr)', gap: 16, marginBottom: 28 }}>
        {[
          ...
          { label: 'Online', val: online, color: '#b8924a', icon: 'ON' },
          { label: 'Warning', val: warning, color: '#e8a020', icon: 'WRN' },
          { label: 'Offline', val: offline, color: '#c85820', icon: 'OFF' },
        ].map(s => (
      ...
      <div className="card" style={{ padding: '20px 24px', marginBottom: 24 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : isTablet ? 'repeat(2, 1fr)' : 'repeat(3, 1fr)', gap: 12 }}>
          {allSensors.map(sensor => {
            const statusCol = sensor.status === 'online' ? '#b8924a' : sensor.status === 'warning' ? '#e8a020' : '#c85820';
            return (
              <div key={sensor.id} style={{ 
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                  <div>
                    <div style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 700, fontSize: 14, color: '#1f2937' }}>{sensor.id}</div>
                    <div style={{ fontSize: 11, color: '#6b7280' }}>{sensor.field}</div>
                  </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <span style={{ fontSize: 11, color: '#1f2937' }}>{sensor.lastPing}</span>
                  </div>
                </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isTablet ? '1fr' : '1fr 1fr', gap: 20 }}>
        <div className="card" style={{ padding: '20px 24px' }}>
          ...
          {[
            ...
            <div key={stat.label} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid rgba(184,146,74,0.08)', fontSize: 13 }}>
              <span style={{ color: '#6b7280' }}>{stat.label}</span>
              <span style={{ fontWeight: 700, color: stat.good ? '#b8924a' : '#e8a020' }}>{stat.val}</span>
            </div>
          ))}
        </div>
        ...
        <div className="card" style={{ padding: '20px 24px' }}>
            ...
            <div key={f.label} style={{ marginBottom: 14 }}>
              ...
              <input 
                ...
                style={{ 
                  ...
                  background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(184,146,74,0.2)',
                  color: '#1f2937', fontSize: 13, outline: 'none', fontFamily: 'Manrope, sans-serif'
                }}
              />
            </div>
          ))}
          <button style={{ 
            ...
            background: 'linear-gradient(135deg, #7d5f2d, #9d7a3d)',
            color: '#1f2937', fontWeight: 700, fontSize: 13, cursor: 'pointer', fontFamily: 'Manrope, sans-serif'
          }}>
            + Pair Sensor
          </button>
        </div>
