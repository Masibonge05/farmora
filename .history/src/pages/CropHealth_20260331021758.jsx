    ...
    <div style={{ padding: isMobile ? '18px 14px' : '28px 32px', maxWidth: 1200 }}>
            ...
            <div key={field.id} className="card" style={{ padding: '20px', textAlign: 'center' }}>
              ...
              <div style={{ position: 'relative', width: 90, height: 90, margin: '0 auto 14px' }}>
                ...
                <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                  <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: '1.3rem', color: col, lineHeight: 1 }}>
                    {(field.ndvi * 100).toFixed(0)}
                  </div>
                  <div style={{ fontSize: 9, color: '#8a9e8a' }}>NDVI</div>
                </div>
              </div>
              <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 13, color: '#f0f4f0' }}>{field.crop}</div>
                ...
                <div key={i} style={{ 
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      ...
                      <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 18, color: sevColor }}>{scan.confidence}%</div>
                    </div>
                  </div>
                  <div style={{ 
      ...
      <div className="card" style={{ padding: '20px 24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : isTablet ? 'repeat(2, 1fr)' : 'repeat(3, 1fr)', gap: 14 }}>
          {[
            { icon: '💉', title: 'Apply Fungicide', field: 'Field B · Urgent', desc: 'Mancozeb 80WP recommended. Apply within 48 hours to prevent spread of leaf spot.', type: 'alert' },
            { icon: '💧', title: 'Emergency Irrigation', field: 'Field D · Critical', desc: 'Soil moisture at 28%. Run drip irrigation for 4 hours at 6L/hr. Check drainage first.', type: 'alert' },
            { icon: '📈', title: 'Harvest Window', field: 'Field A · Opportunity', desc: 'Maize approaches optimal brix in 12 days. Pre-book market at current R4,280/ton.', type: 'info' },
          ].map(r => (
            <div key={r.title} style={{ 
              ...
              <div style={{ fontWeight: 700, fontSize: 13, color: '#f0f4f0', marginBottom: 2 }}>{r.title}</div>
