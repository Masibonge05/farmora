  ...
  if (active && payload?.length) {
    return (
      <div style={{ background: '#ffffff', borderRadius: 8, padding: '10px 14px', fontSize: 12, boxShadow: '0 10px 24px rgba(15,23,42,0.12)' }}>
        <div style={{ color: 'var(--text-strong)', fontWeight: 700, marginBottom: 6 }}>{label}</div>
        {payload.map((p, i) => (
  ...
  return (
    <div style={{ padding: isMobile ? '18px 14px' : '28px 32px', width: '100%' }}>
      <div style={{ marginBottom: 28 }}>
          ...
          <div style={{ fontSize: 14, color: 'var(--text-soft)' }}>Real-time IoT sensor data - Moisture, nutrients, pH, and temperature</div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : isTablet ? 'repeat(2, 1fr)' : 'repeat(4, 1fr)', gap: 16, marginBottom: 24 }}>
        {soilNodes.map(node => {
          ...
          const fieldTag = getFieldStyle(node.field);
          return (
            <div key={node.id} className="card" style={{ padding: '18px 20px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <div style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 700, fontSize: 14, color: 'var(--text-strong)' }}>{node.field}</div>
                    <span style={{ width: 8, height: 8, borderRadius: 999, background: fieldTag.color, boxShadow: `0 0 0 3px ${fieldTag.bg}` }} />
                  </div>
                  <div style={{ fontSize: 10, color: 'var(--text-soft)', marginTop: 1 }}>{node.id}</div>
                </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isTablet ? '1fr' : '1.4fr 1fr', gap: 20, marginBottom: 24 }}>
        ...
        <div className="card" style={{ padding: '20px 24px' }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--text-soft)', marginBottom: 16 }}>Moisture %, Temperature C, pH</div>
        ...
        <div className="card" style={{ padding: '20px 24px' }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--text-soft)', marginBottom: 20 }}>N-P-K analysis - Requires attention</div>
          ...
          <div style={{ marginTop: 16, padding: '12px 14px', background: 'rgba(100,116,139,0.08)', borderRadius: 10, boxShadow: 'inset 0 1px rgba(100,116,139,0.22), inset 0 -1px rgba(100,116,139,0.12)' }}>
            <div style={{ fontSize: 12, color: 'var(--text-soft)', fontWeight: 600, marginBottom: 4 }}>Fertiliser Recommendation</div>
          ...
          {soilNodes.map(node => {
            ...
            return (
              <div key={node.id} style={{ 
                ...
                boxShadow: `inset 0 1px ${needsIrrigation ? 'rgba(111,155,134,0.3)' : 'rgba(100,116,139,0.2)'}, inset 0 -1px ${needsIrrigation ? 'rgba(111,155,134,0.12)' : 'rgba(100,116,139,0.1)'}`,
              }}>
                <div style={{ fontWeight: 700, fontSize: 13, color: 'var(--text-strong)', marginBottom: 4 }}>{node.field}</div>
                <div style={{ fontSize: 14, margin: '8px 0', color: 'var(--text-soft)', fontWeight: 700 }}>IR</div>
                <div style={{ fontSize: 22, fontFamily: 'Outfit, sans-serif', fontWeight: 800, color: urgencyColor }}>{node.moisture}%</div>
                <div style={{ fontSize: 11, color: 'var(--text-soft)', marginBottom: 8 }}>current moisture</div>
                <div style={{ fontSize: 11, color: urgencyColor, fontWeight: 600, marginBottom: 4 }}>{urgency}</div>
                <div style={{ fontSize: 11, color: 'var(--text-soft)' }}>Next: <span style={{ color: 'var(--text-strong)' }}>{nextIrrigation}</span></div>
              </div>
            );
          })}
        </div>
