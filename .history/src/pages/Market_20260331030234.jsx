...
const CustomTooltip = ({ active, payload, label }) => {
  if (active && payload?.length) {
    return (
      <div style={{ background: 'rgba(15,26,15,0.95)', border: '1px solid rgba(184,146,74,0.3)', borderRadius: 8, padding: '10px 14px', fontSize: 12 }}>
          ...
          <div key={m.crop} className="card" style={{ padding: '20px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ 
                ...
                fontSize: 13, fontWeight: 700, color: m.trend === 'up' ? '#b8924a' : '#f08060'
              }}>
                {m.trend === 'up' ? '+' : '-'} {m.change}
              </div>
              <div style={{ fontSize: 11, color: '#9d7a3d' }}>{m.buyers} buyers</div>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isTablet ? '1fr' : '1.2fr 1fr', gap: 20, marginBottom: 24 }}>
        ...
        <div className="card" style={{ padding: '20px 24px' }}>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={priceHistory} barGap={2}>
              ...
              <Bar dataKey="maize" name="Maize" fill="#b8924a" radius={[3,3,0,0]} />
        ...
        <div className="card" style={{ padding: '20px 24px' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: 20 }}>
            ...
            <div style={{ fontSize: 11, color: '#9d7a3d', marginTop: 8, fontWeight: 600 }}>FM-A-2026-0330-MAIZE</div>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {[
              ...
              <div key={r.label} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid rgba(184,146,74,0.1)', fontSize: 12 }}>
            ...
            <div key={buyer.id} className="card" style={{ padding: '16px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                  ...
                  {buyer.verified && <span style={{ fontSize: 12, color: '#8d6b33', fontWeight: 700 }}>VER</span>}
                </div>
              ...
              <div style={{ textAlign: 'right', marginLeft: 16 }}>
                <div style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 800, fontSize: '1.3rem', color: '#b8924a' }}>{buyer.rating}</div>
                <div style={{ fontSize: 10, color: '#6b7280' }}>Rating</div>
                <button style={{ marginTop: 10, background: 'rgba(184,146,74,0.2)', border: '1px solid rgba(184,146,74,0.4)', color: '#b8924a', borderRadius: 8, padding: '6px 14px', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'Manrope, sans-serif' }}>
                  Contact
                </button>
              </div>
            </div>
