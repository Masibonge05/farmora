...
const uiText = {
  en: {
    ...
    cropAnalysisDetail: "Disease, nutrient, and growth-stage diagnostics",
    locationAwareDetail: "Advice tuned to your field and local weather",
    multilingualDetail: "Ask and respond in local languages naturally",
    back: "Back",
    newChat: "New chat",
...
function CapabilityLineItem({ icon, title, detail }) {
  return (
    <div className="capability-item">
      ...
      <div style={{ minWidth: 0 }}>
        <div style={{ fontSize: 13, fontWeight: 800, color: theme.greenDark, lineHeight: 1.2, marginBottom: 2 }}>
          {title}
        </div>
...
export default function FarmoraAIPage({ onBack }) {
  ...
  useEffect(() => {
    ...
    // Keep the viewport stable when opening the AI page (especially with restored chat history).
      ...
      <div style={{ maxWidth: 1320, margin: "0 auto" }}>
        ...
        >
          ...
          >
            <div>
              ...
              <div className="capability-strip" style={{ marginTop: 10, maxWidth: 760 }}>
                <CapabilityLineItem
                  icon={<ShieldCheck size={14} strokeWidth={1.8} />}
                  title={ui.cropAnalysis}
                  detail={ui.cropAnalysisDetail}
                />
                <CapabilityLineItem
                  icon={<MapPin size={14} strokeWidth={1.8} />}
                  title={ui.locationAware}
                  detail={ui.locationAwareDetail}
                />
                <CapabilityLineItem
                  icon={<Languages size={14} strokeWidth={1.8} />}
                  title={ui.multilingual}
                  detail={ui.multilingualDetail}
                />
              </div>
        ...
        >
          ...
          <div style={{ display: "grid", gap: 14 }}>
            <InfoCard title={ui.currentDraft} icon={<Activity size={18} />}>
            ...
            <InfoCard title={ui.whatFarmoraReturns} icon={<Bot size={18} />}>
              <div style={{ fontSize: 13 }}>{ui.whatFarmoraReturnsText}</div>
            </InfoCard>

            <InfoCard title={ui.responseStatus} icon={<CheckCircle2 size={18} />}>
              {latestAssistant ? (
                <div>
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 10, fontSize: 12 }}>
                    <Pill icon={<CheckCircle2 size={14} />}>{ui.requestCompleted}</Pill>
                    <Pill icon={<MapPin size={14} />}>{locationLabel}</Pill>
                    <Pill icon={<Languages size={14} />}>{activeLanguage.label}</Pill>
                    <Pill
                      icon={audioBase64 ? <Mic size={14} /> : <MicOff size={14} />}
                ...
                <div style={{ fontSize: 13 }}>{ui.waiting}</div>
              )}
            </InfoCard>
          </div>
        </div>
