  ...
  return (
    <div style={{ padding: isMobile ? '18px 14px' : '28px 32px', maxWidth: 1100 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : isTablet ? 'repeat(2, 1fr)' : 'repeat(4, 1fr)', gap: 16, marginBottom: 28 }}>
        {[
          { label: 'Total Nodes', val: allSensors.length, color: '#f0f4f0', icon: '📡' },
          { label: 'Online', val: online, color: '#7ec87e', icon: '✅' },
          { label: 'Warning', val: warning, color: '#e8a020', icon: '⚠️' },
          { label: 'Offline', val: offline, color: '#c85820', icon: '❌' },
        ].map(s => (
          <div key={s.label} className="card" style={{ padding: '18px 20px', display: 'flex', alignItems: 'center', gap: 14 }}>
            <div style={{ fontSize: 28 }}>{s.icon}</div>
            <div>
              <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: '1.8rem', color: s.color, lineHeight: 1 }}>{s.val}</div>
              <div style={{ fontSize: 12, color: '#8a9e8a', marginTop: 2 }}>{s.label}</div>
            </div>
          </div>
        ))}
      </div>
            ...
            return (
              <div key={sensor.id} style={{ 
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                  <div>
                    <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 14, color: '#f0f4f0' }}>{sensor.id}</div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <span style={{ fontSize: 11, fontWeight: 700, color: signalColor[sensor.signal] || '#8a9e8a' }}>{sensor.signal}</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: 11, color: '#8a9e8a', display: 'flex', alignItems: 'center', gap: 4 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isTablet ? '1fr' : '1fr 1fr', gap: 20 }}>
        <div className="card" style={{ padding: '20px 24px' }}>
          ...
          {[
            ...
            <div key={stat.label} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid rgba(74,158,74,0.08)', fontSize: 13 }}>
              ...
              <span style={{ fontWeight: 700, color: stat.good ? '#7ec87e' : '#e8a020' }}>{stat.val}</span>
            </div>
          ))}
        </div>
        ...
        <div className="card" style={{ padding: '20px 24px' }}>
          ...
          <button style={{ 
            ...
            color: '#f0f4f0', fontWeight: 700, fontSize: 13, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif'
          }}>
            + Pair Sensor
          </button>
        </div>
