  ...
  return (
    <div style={{ marginBottom: 14 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
        ...
        <span style={{ fontSize: 12, fontWeight: 700, color: isLow ? '#c85820' : '#7ec87e' }}>{value}%</span>
      </div>
          ...
          return (
            <div key={node.id} className="card" style={{ padding: '18px 20px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
                <div>
                  <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 14, color: '#f0f4f0' }}>{node.field}</div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                {[
                  { label: 'Moisture', val: `${node.moisture}%`, color: '#80c0f0', icon: '💧' },
                  { label: 'pH Level', val: node.pH, color: node.pH < 6.0 ? '#e8a020' : '#7ec87e', icon: '🧪' },
                  { label: 'Temp', val: `${node.temp}°C`, color: '#e8a020', icon: '🌡' },
                  { label: 'N Level', val: `${node.nitrogen}%`, color: '#7ec87e', icon: '🌿' },
                ].map(m => (
                  <div key={m.label} style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 8, padding: '8px 10px', textAlign: 'center' }}>
                    ...
                    <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 15, color: m.color }}>{m.val}</div>
          ...
          {soilNodes.map(node => {
            ...
            return (
              <div key={node.id} style={{ 
                ...
                <div style={{ fontWeight: 700, fontSize: 13, color: '#f0f4f0', marginBottom: 4 }}>{node.field}</div>
                <div style={{ fontSize: 26, margin: '8px 0' }}>💧</div>
                <div style={{ fontSize: 22, fontFamily: 'Syne, sans-serif', fontWeight: 800, color: urgencyColor }}>{node.moisture}%</div>
