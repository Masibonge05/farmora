...
export default function Overview({ onNav }) {
  ...
  return (
    <div className="dashboard-frame" style={{ padding: isMobile ? '18px 14px' : '28px 32px', width: '100%' }}>
      {!isMobile && (
        <div className="insight-stack" style={{ marginBottom: 18 }}>
          <div className="insight-card main single">
            <div className="insight-title">Account Insights</div>
            <div className="insight-copy">Automation boosted operational response times by 48.3 hours this month.</div>
            <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
              <span className="mini-chip">Sync 99.2%</span>
              <span className="mini-chip">Anomalies 3</span>
              <span className="mini-chip">Auto tasks 128</span>
            </div>
          </div>
        </div>
      )}
          ...
          {weatherData.forecast.map(f => (
            <div key={f.day} className="card" style={{ padding: '10px 14px', marginBottom: 6, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <span style={{ fontSize: 12, color: 'var(--text-soft)' }}>{f.day}</span>
              </div>
              <div style={{ fontSize: 11, color: '#6f9b86' }}> {f.rain}</div>
              <div style={{ fontSize: 12, color: 'var(--text-strong)', fontWeight: 600 }}>{f.high} / {f.low}</div>
            </div>
          ))}
        </div>
      </div>
