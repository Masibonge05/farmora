...
export default function Weather() {
  ...
  const weather = useMemo(
    () => ({
      current: weatherData?.current || { temp: 24, humidity: 58, wind: 12, condition: 'Partly Cloudy', icon: 'CL' },
  ...
  const { current, hourly: hourlyData, extended: extendedData, location, sunrise } = weather;
  const maxTemp = Math.max(...extendedData.map((d) => d.high));
      ...
      <div style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 16, minWidth: 0 }}>
          {[
            ...
            { icon: 'S', label: 'Sunrise',    val: sunrise },
          ].map(m => (
