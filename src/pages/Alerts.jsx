      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 17, color: '#f0f4f0' }}>Start Irrigation</div>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={onCancel} style={{
            ...
            color: '#8a9e8a', fontSize: 13, fontWeight: 600, fontFamily: 'DM Sans, sans-serif',
          }}>Cancel</button>
          <button onClick={onConfirm} style={{
            flex: 2, padding: '11px', borderRadius: 10, cursor: 'pointer', border: 'none',
            background: 'linear-gradient(135deg, #1a5a8a, #3a8cc8)',
            color: '#f0f4f0', fontSize: 13, fontWeight: 700, fontFamily: 'DM Sans, sans-serif',
...
function Toast({ message, type = 'success', onDone }) {
  ...
  const isProcessing = message.startsWith('⏳');
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {isProcessing ? <span style={{ fontSize: 18 }}>⏳</span> : <CheckCircle size={20} color={color} />}
      </div>
      <div style={{ flex: 1 }}>
        <div style={{ fontWeight: 700, fontSize: 13, color: '#f0f4f0' }}>
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        setAlertList(prev => prev.map(a =>
          a.id === id
            ? { ...a, type: 'success', message: 'Irrigation started — Field D drip system running at 6 L/hr for 4 hours. Est. completion: 22:30.', time: 'Just now', icon: '💧' }
            : a
        ));
        setProcessing(false);
