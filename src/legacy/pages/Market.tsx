...
import { marketPrices as fallbackMarketPrices } from '../data/dummyData';
import { useDashboardData } from '../data/dashboard-provider';
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from 'recharts';
import { ArrowDownRight, ArrowUpRight, CircleCheck, Plus, ShoppingCart, UserRound } from 'lucide-react';
      ...
      <div style={{ background: 'rgba(15,26,15,0.95)', border: '1px solid rgba(74,158,74,0.3)', borderRadius: 8, padding: '10px 14px', fontSize: 12 }}>
        ...
        {payload.map((p, i) => (
          <div key={i} style={{ color: p.color, display: 'flex', gap: 8, marginBottom: 2 }}>
            <span>{p.name}:</span><span style={{ fontWeight: 600 }}>R{p.value.toLocaleString()}</span>
          </div>
        ))}
      </div>
    );
...
export default function Market() {
  ...
  const marketPrices = Array.isArray(data?.marketPrices) ? data.marketPrices : fallbackMarketPrices;
  return (
    <div style={{ padding: '28px 32px', maxWidth: 1200 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <ShoppingCart size={20} color="#7ec87e" />
          ...
          <button style={{ 
            ...
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}><Plus size={14} /> List Produce</span>
          </button>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 24 }}>
        {marketPrices.map(m => (
          <div key={m.crop} className="card" style={{ padding: '20px' }}>
            ...
            <div style={{ fontFamily: 'Poppins, sans-serif', fontWeight: 600, fontSize: '1.6rem', color: '#f0f4f0', lineHeight: 1 }}>{m.price}</div>
            <div style={{ fontSize: 12, color: '#8a9e8a', marginTop: 2, marginBottom: 10 }}>{m.unit}</div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ 
                display: 'flex', alignItems: 'center', gap: 4,
                fontSize: 13, fontWeight: 600, color: m.trend === 'up' ? '#7ec87e' : '#f08060'
              }}>
                {m.trend === 'up' ? <ArrowUpRight size={14} /> : <ArrowDownRight size={14} />} {m.change}
              </div>
              <div style={{ fontSize: 11, color: '#4a9e4a', display: 'flex', alignItems: 'center', gap: 4 }}><UserRound size={11} /> {m.buyers} buyers</div>
            </div>
          </div>
        ))}
      </div>
      ...
      {/* Verified buyers */}
      <div>
        <div style={{ fontFamily: 'Poppins, sans-serif', fontWeight: 600, fontSize: 15, color: '#f0f4f0', marginBottom: 14 }}>
          Verified Buyers
        </div>
            ...
            <div key={buyer.id} className="card" style={{ padding: '16px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                  <div style={{ fontFamily: 'Poppins, sans-serif', fontWeight: 600, fontSize: 14, color: '#f0f4f0' }}>{buyer.name}</div>
                  {buyer.verified && <CircleCheck size={14} color="#7ec87e" />}
                </div>
              ...
              <div style={{ textAlign: 'right', marginLeft: 16 }}>
                <div style={{ fontFamily: 'Poppins, sans-serif', fontWeight: 600, fontSize: '1.3rem', color: '#7ec87e' }}>{buyer.rating}</div>
                <div style={{ fontSize: 10, color: '#8a9e8a' }}>Rating</div>
                <button style={{ marginTop: 10, background: 'rgba(74,158,74,0.2)', border: '1px solid rgba(74,158,74,0.4)', color: '#7ec87e', borderRadius: 8, padding: '6px 14px', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'Poppins, sans-serif' }}>
                  Contact
                </button>
              </div>
            </div>
